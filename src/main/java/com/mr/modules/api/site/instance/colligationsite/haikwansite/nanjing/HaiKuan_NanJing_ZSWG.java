...
public class HaiKuan_NanJing_ZSWG extends SiteTaskExtend_CollgationSite_HaiKWan {
    ...
    private String source = "南京海关";
    private String subject = "南京海关走私违规行政处罚";
    private String judgeAuth = "南京海关";
    ...
    private String getName(String str){
        ...
        try{
            if(list.size()>0){
                ...
                str = str.replace("张关缉违字(2018)00奥伯尼应用技术(苏州)有限公司","张关缉违字(2018)0030号奥伯尼应用技术(苏州)有限公司");
                if(!str.contains("字") && !str.contains("号")){
                    nameInfo = str;
                }else{
                    if(str.contains("公司")){
                        nameInfo = str.substring(str.indexOf("号")+1,str.indexOf("公司")+2).replace("当事人：","").replace("当事人","");
    ...
    private String getInfo(Map<String,String> map){
        ...
        String tail = map.get("attachmentName").substring(attachmentName.lastIndexOf("."));
        ...
        if(tail.toLowerCase().equalsIgnoreCase(".pdf")){
            ...
            try{
                if(tail.toLowerCase().contains(".tif")){
                    resultStr = BaiduOCRUtil.getTextStrFromTIFFile(filePath,attachmentName);
                    if(!resultStr.contains("海关")){//若连海关都没识别到，判断为识别有误
                        resultStr = "";
                    }
                }else{
    ...
    public String parsePdfInfo(String filePath,String attachmentName) throws Exception {
        ...
        try{
            ...
            log.error("解析pdf异常,pdf文件为{}",filePath+File.separator+attachmentName);
            e.printStackTrace();
        }
