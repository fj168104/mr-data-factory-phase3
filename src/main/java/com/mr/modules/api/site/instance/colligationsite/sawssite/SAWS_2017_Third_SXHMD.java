...
/**
 * 站点：国家安全生产监督管理总局网站
 * url：http://old.chinasafety.gov.cn/yfyj/yhpg/201712/t20171205_124478.shtml
 * 主题：2017年第三批安全生产失信联合惩戒“黑名单”单位及其人员名单
 * 属性：单位名称、注册地址、统一社会信用代码、主要负责人、身份证号、失信行为简况、信息报送机关、纳入理由、发布日期
 * 提取：TODO 单位名称 发布日期  信息报送机关 
 */
@Slf4j
@Component("swas_2017_third_sxhmd")
@Scope("prototype")
public class SAWS_2017_Third_SXHMD extends SiteTaskExtend_CollgationSite {
    @Override
    protected String execute() throws Throwable {

        log.info("开始处理国家安全生产监督管理总局网站-2017年度第三批失信黑名单");
        String reqUrl = "http://old.chinasafety.gov.cn/yfyj/yhpg/201712/t20171205_124478.shtml";
        String filePath = OCRUtil.DOWNLOAD_DIR + File.separator  +"swas_2017_third_sxhmd"+File.separator+ MD5Util.encode(reqUrl);
     ...
     //   log.info("html正文：{}",htmlText);
        ...
        String wenHao = "2017年第16号";
        String publishDate = "2017年12月4日";
        String htmlFileName = title+".html";
        //下载html文件
        saveFile(htmlPage,htmlFileName,filePath);
        //下载附件
        List list = htmlPage.getByXPath("//*[@id=\"nrBox\"]/font/p[4]/span[1]/a");
        ...
        for(Map<String, Object> map : DataList){
            ...
            adminPunish.setSubject("2017年第三批安全生产失信联合惩戒“黑名单”单位及其人员名单");
            adminPunish.setUniqueKey(reqUrl+"@"+entName+"@"+i+"@"+publishDate);
            adminPunish.setUrl(reqUrl);
            adminPunish.setObjectType("01");
            adminPunish.setEnterpriseName(entName);
            if(enterpriseCode1.contains("组织机构代码：") && enterpriseCode1.contains("注册号：")){
                String[] strs = enterpriseCode1.split("：");
                String code2 = strs[strs.length-1];
                String code3 = strs[1].substring(0,strs[1].indexOf("注册号"));
                adminPunish.setEnterpriseCode2(code2);
                adminPunish.setEnterpriseCode3(code3);
            }else if(enterpriseCode1.contains("营业执照：")){
                String[] strs = enterpriseCode1.split("：");
                String code2 = strs[1];
                adminPunish.setEnterpriseCode2(code2);
            }else if(enterpriseCode1.contains("信用代码")){
                adminPunish.setEnterpriseCode1(enterpriseCode1.substring(enterpriseCode1.indexOf("信用代码")+4));
            }else{
                adminPunish.setEnterpriseCode1(enterpriseCode1.replace("’",""));
            }
        ...
        webClient.close();
        log.info("结束处理国家安全生产监督管理总局网站-2018年度");
        return "";
    }
