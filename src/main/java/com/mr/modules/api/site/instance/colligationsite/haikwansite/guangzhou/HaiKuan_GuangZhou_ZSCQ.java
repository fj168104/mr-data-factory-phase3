 ...
 * @DateTime 2018-08-05
 * 主题：广州海关知识产权行政处罚
 * url:http://guangzhou.customs.gov.cn/guangzhou_customs/381558/381601/381603/381605/index.html
 * 属性：企业名称, 执行文号, 处罚事由, 处罚依据, 处罚结果, 认定机关, 发布日期
 */
@Slf4j
@Scope("prototype")
@Component("haikuan_guangzhou_zscq")
public class HaiKuan_GuangZhou_ZSCQ extends SiteTaskExtend_CollgationSite_HaiKWan {
    @Autowired
    SiteParams siteParams;
    private String source = "广州海关";
    private String subject = "广州海关知识产权行政处罚";
    private String judgeAuth = "广州海关";

    @Override
    protected String execute() throws Throwable {
        String ip = "";
        String port = "";
    //    String source = "广州海关知识产权行政处罚";
        String area = "guangzhou";//区域为：广州
        String baseUrl = "http://guangzhou.customs.gov.cn";
        String url = "http://guangzhou.customs.gov.cn/guangzhou_customs/381558/381601/381603/381605/index.html";
    ...
    public void parseText(String resultStr,Map<String,String> map){
        ...
        String judgeNo = getJudgeNo(title);
    ...
    private String getName(String str){
        ...
        nameInfo = nameInfo.replace("肇关缉违字(2017009","");
    ...
    private String getJudgeNo(String str){
        ...
        try{
            if(str.contains("字") && str.contains("号")){
                judgeNo = str.substring(str.lastIndexOf("关")-1,str.indexOf("号")+1);
            }else{
                judgeNo = str.substring(str.lastIndexOf("关")-1)+"号";
            }
        }catch (Exception e){
            log.error("获取处罚文号失败，title为 {}",str);
            log.error(e.getMessage());
        }
    ...
    private String getInfo(Map<String,String> map){
        ...
        if(tail.toLowerCase().equalsIgnoreCase(".jpg") || tail.toLowerCase().equalsIgnoreCase(".png")){
            try{
                ...
                log.error("jpg解析失败，URL为 {}",url);
                log.error(e.getMessage());
            }

        }else if(tail.toLowerCase().equalsIgnoreCase(".pdf")){
            try {
