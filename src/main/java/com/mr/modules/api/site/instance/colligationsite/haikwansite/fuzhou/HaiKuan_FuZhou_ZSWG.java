...
public class HaiKuan_FuZhou_ZSWG extends SiteTaskExtend_CollgationSite_HaiKWan {
    ...
    public void extractWebData(Map<String,String> map){
        ...
        adminPunish.setSubject("福州海关走私违规行政处罚");
        ...
        Pattern pattern = Pattern.compile("[\\u4e00-\\u9fa5]+[\\s]{0,}[关,洲][\\s]{0,}[知,查,郴,机,缉,违,罚,公,处]{1,}[\\s]{0,}[字][\\s]{0,}\\[[\\s]{0,}[0-9][\\s]{0,}[0-9][\\s]{0,}[0-9][\\s]{0,}[0-9][\\s]{0,}\\][\\s]{0,}[0-9]{0,}[\\s]{0,}[0-9]{0,}[\\s]{0,}[0-9]{0,}[\\s]{0,}[\\s]{0,}[-]{0,}[0-9]{0,}[\\s]{0,}[号]");
        ...
        for(String str : textArr){
            if(str.contains("：")){
                ...
                if(str.contains("发布主题")&&str.contains("处罚决定书")&&str.contains("字")&&str.contains("号")){
                    adminPunish.setJudgeNo(str.replaceAll(".*处罚决定书",""));
                }
                if(str.contains("发布主题")&&str.contains("公司")&&str.contains("关于")&&str.contains("处罚决定书")){
                    adminPunish.setJudgeAuth(strArr[1].replaceAll("关于.*",""));
                    adminPunish.setEnterpriseName(str.replaceAll(".*关于","").replaceAll("公司.*","公司"));
                }
            }
        }
