...
public class HaiKuan_HangZhou_ZSWG extends SiteTaskExtend_CollgationSite_HaiKWan {
    ...
    public void extractWebData(Map<String,String> map){
        ...
        adminPunish.setSubject("杭州海关走私违规行政处罚");
        ...
        for(String str : textArr){
            if(str.contains("：")){
                ...
                if(adminPunish.getJudgeAuth().equals("")&&strArr[0].contains("发布主题")&&strArr[1].contains("行政处罚决定书")){
                    adminPunish.setJudgeAuth(strArr[1].replaceAll("行政处罚决定书.*",""));
                }
            }

            if(str.contains("发布主题")&&str.contains("行政处罚决定书")){
