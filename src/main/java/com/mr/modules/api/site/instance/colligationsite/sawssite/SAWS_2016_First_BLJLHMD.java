 ...
 */
@Slf4j
@Component("swas_2016_first_bljlhmd")
@Scope("prototype")
public class SAWS_2016_First_BLJLHMD extends SiteTaskExtend_CollgationSite {
    ...
    private String hashKey = OCRUtil.DOWNLOAD_DIR + File.separator + "swas_2016_first_bljlhmd" + File.separator + MD5Util.encode(url);
    ...
    protected String execute() throws Throwable {
        ...
        //1、发生重特大生产安全事故的2家生产经营单位
        AdminPunish adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("辽宁连山钼业集团兴利矿业有限公司");
        adminPunish.setEnterpriseCode2("211402004015816");
        adminPunish.setPunishReason("发生重特大生产安全事故");
        adminPunish.setPunishAccording("2015年12月17日，辽宁连山钼业集团兴利矿业有限公司井下发生重大火灾事故，造成17人死亡、17人受伤，直接经济损失2199.1万元。辽宁省安全生产监督管理局对该企业处以500万元罚款");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("河南省通许县通安烟花爆竹有限公司");
        adminPunish.setPunishReason("发生重特大生产安全事故");
        adminPunish.setPunishAccording("2016年1月14日上午10时48分左右，河南省开封市通许县通安烟花爆竹有限公司将老厂房出租给他人，在违法生产烟花爆竹（超规格双响类产品）过程中发生爆炸，造成10人死亡、7人受伤。该起事故正在调查处理中");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        //2、瞒报生产安全事故的生产经营单位
        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("唐山燕山电力设备检修有限公司");
        adminPunish.setEnterpriseCode2("130293000015867");
        adminPunish.setPunishReason("瞒报生产安全事故");
        adminPunish.setPunishAccording("2015年10月12日 ，唐山燕山电力设备检修公司在神华国能宁夏煤电有限公司鸳鸯湖电厂进行锅炉检修时违章作业，发生倾砸事故，造成1人死亡。事故发生后，施工单位未向建设单位和政府有关职能部门报告，导致事故存在瞒报行为");
        ...
        //3、存在其他严重违法违规行为
        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("山东省潍坊长兴化工有限公司");
        adminPunish.setPunishReason("存在其他严重违法违规行为");
        adminPunish.setPunishAccording("违法出租生产装置和设备、不具备安全生产条件组织生产、现场安全管理缺失");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("四川省金路树脂股份有限公司");
        adminPunish.setPunishReason("存在其他严重违法违规行为");
        adminPunish.setPunishAccording("违规进入受限空间作业、教育培训缺失");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("江西省上饶市广丰县鸿盛花炮制造有限公司");
        adminPunish.setPunishReason("存在其他严重违法违规行为");
        adminPunish.setPunishAccording("多股东各自独立组织生产，库房超量储存");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("广西区钦州市浦北县泉水烟花基地");
        adminPunish.setPunishReason("存在其他严重违法违规行为");
        adminPunish.setPunishAccording("将闲置的工房出租给他人、违法组织生产组合烟花，并故意瞒报事故");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("广西壮族自治区玉林市博白县龙潭炮竹厂");
        adminPunish.setPunishReason("存在其他严重违法违规行为");
        adminPunish.setPunishAccording("违反停产整顿指令组织生产、违规在称量工房内同时称量氧化剂和还原剂等");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);

        adminPunish = createAdminPunish();
        adminPunish.setEnterpriseName("广西壮族自治区玉林市博白县松旺烟花厂");
        adminPunish.setPunishReason("存在其他严重违法违规行为");
        adminPunish.setPunishAccording("改变工房用途、私建工房、制度不落实、内部管理混乱");
        adminPunish.setUniqueKey(getUniqueKey(adminPunish));
        adminPunishMapper.insert(adminPunish);
